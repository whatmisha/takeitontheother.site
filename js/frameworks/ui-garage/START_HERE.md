...
This is the stable `2.0.0` release. Runtime, assets and complete SVG and Canvas
...
```html
<link rel="stylesheet" href="../infra/ui-garage/css/framework.css">
<link rel="stylesheet" href="../infra/ui-garage/css/ui-contract.css">
...
```js
import { ToolUiController, defineTool } from '../infra/ui-garage/src/index.js';
```
...
`ToolUiController` is the preferred owner for button captions, keyboard routes
and promise-based export feedback. It shows the shortcut only on the primary
export. `GeneratorHost` is available from the same public barrel for adapting
an existing page; a new tool should normally start with `defineTool`.

## Project placement

Copy this directory once to `infra/ui-garage/`. Put every tool directly under
the repository root as its own sibling directory. Do not put tools inside UI
Garage and do not link UI Garage back to the repository it was copied from.
The complete rules and tree are in `PROJECT_LAYOUT.md`.

## Choose a starting point

For a production-shaped graphics generator, use the nearest complete starter
as a reference and replace its domain settings and render functions:
...
If a starter is copied out of this folder, change its relative imports to the
project copy at `../infra/ui-garage/`.
Assets loaded by the Canvas starter are session-only unless the new tool defines
and documents its own persistence format.

From the repository root, create a deliberately minimal clean SVG or Canvas
scaffold:

```sh
node infra/ui-garage/scripts/create-tool.mjs ./my-tool --id my-tool --renderer svg --name "My Tool"
```
