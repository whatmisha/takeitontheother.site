...
test('public barrel exports the documented framework surface', async () => {
    ...
    const expected = [
        ...
        'SvgTarget', 'TextToPath', 'ToolUiController', 'TooltipService', 'UnifiedColorPicker',
        'UnifiedUiController', 'WobblyEffect', 'ZoomPanManager', 'bindPageLifecycle', 'connectFileInput',
        'createCollapseButton', 'defineTool', 'downloadBlob', 'downloadCanvas', 'fileMatchesAccept',
        'initActionDocks', 'initPresetMenuKeyboards', 'initUnifiedUi', 'mountGenerator', 'normalizeControls',
        'resolveApplicationCapabilities', 'seedToUint32', 'svgDocumentString'
    ];
...
test('every public resource-owning controller exposes destroy()', async () => {
    ...
    for (const name of [
        ...
        'DialogHost', 'ExportFeedbackController', 'FileIntakeController', 'HistoryBridge', 'MobileBootstrap',
        ...
        'ToolUiController', 'TooltipService', 'UnifiedColorPicker', 'UnifiedUiController', 'ZoomPanManager'
...
test('every source module has one explicit owner and API surface classification', async () => {
    ...
    assert.equal(new Set(declared).size, 49);
