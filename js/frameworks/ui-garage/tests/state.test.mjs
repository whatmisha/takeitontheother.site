...
test('ApplicationShell uses short URLs only for clean bundled presets', async () => {
    ...
    globalThis.location = {
        href: 'https://example.test/tools/example/?old=1',
    ...
    try {
        ...
        assert.equal(shortUrl, 'https://example.test/tools/example/?preset=basic-look');

        shell.presets.isDirty = true;
        const fullUrl = await shell._buildShareUrl('Basic Look', { useCurrent: true });
        assert.match(fullUrl, /^https:\/\/example\.test\/tools\/example\/\?old=1#p=v1\./);
    } finally {
