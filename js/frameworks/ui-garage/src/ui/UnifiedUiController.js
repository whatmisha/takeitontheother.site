...
export class UnifiedUiController {
    constructor({
        ...
        this.summaryProviders = Object.freeze({ ...(profile?.summaries || summaryProviders) });
        this.extraShortcutRows = normalizeShortcutRows(profile?.shortcuts?.() || shortcutRows);
    ...
    ensureSummaries() {
        Object.keys(this.summaryProviders).forEach(panelId => {
            ...
            summary.className = 'panel-params ui-panel-summary';
    ...
    handleClick(event) {
        ...
        if (exportButton?.dataset.exportFeedback !== 'explicit' && exportButton?.matches('[data-action-dock-primary-export], [data-action-dock-json-export], [id^="export"], [id^="generate"], [id^="download"]')) {
