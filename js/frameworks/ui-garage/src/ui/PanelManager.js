...
export class PanelManager {
    ...
    registerPanel(panelId, config = {}) {
        ...
        this._listen(panel, 'mousedown', () => this.bringToFront(panelId));
    }
    ...
    initCollapse({ iconSelector = '.collapse-icon', panelSelector = '.controls-panel' } = {}) {
        document.querySelectorAll(iconSelector).forEach((icon) => {
            ...
            if (this._collapseIcons.has(icon)) return;
            this._collapseIcons.add(icon);
            ...
            this._listen(icon, 'click', toggle);
            this._listen(icon, 'keydown', (e) => {
    ...
    destroy() {
        ...
        for (const icon of this._collapseIcons) {
            if (icon.dataset) delete icon.dataset.collapseBound;
        }
        this._collapseIcons.clear();
        this.panels.clear();
        this.expandedPanelSnapshot = null;
    }
}
