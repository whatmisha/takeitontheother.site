...
export class SliderController {
    ...
    initSlider(sliderId, config) {
        ...
        // Event handlers
        this._listen(sliderData, slider, 'input', (e) => this.handleSliderInput(sliderId, e));
        // Removed input handler for valueInput - changes applied only on blur or Enter
        this._listen(sliderData, valueInput, 'keydown', (e) => this.handleKeyDown(sliderId, e));
        this._listen(sliderData, valueInput, 'focus', (e) => e.target.select());
        this._listen(sliderData, valueInput, 'blur', (e) => this.handleValueBlur(sliderId, e));
    }
        ...
        switch (event.key) {
            case 'ArrowUp':
                if (event.shiftKey && shiftStep > 0) {
                    // Shift is an exact additive coarse step, not grid snapping.
    ...
    /** Update visible controls without mutating Settings or preset state. */
    ...
    _listen(sliderData, target, type, listener, options) {
        target.addEventListener(type, listener, options);
        sliderData.cleanups.push(() => target.removeEventListener(type, listener, options));
    }

    destroySlider(sliderId) {
        const sliderData = this.sliders.get(sliderId);
        if (!sliderData) return false;
        for (const cleanup of sliderData.cleanups.splice(0).reverse()) cleanup();
        this.sliders.delete(sliderId);
        return true;
    }

    destroy() {
        for (const sliderId of [...this.sliders.keys()]) this.destroySlider(sliderId);
        this.isUpdating = false;
    }
}
