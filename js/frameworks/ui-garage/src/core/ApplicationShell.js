...
import { resolveApplicationCapabilities } from './ApplicationCapabilities.js';
...
export class ApplicationShell {
    ...
    async _initialize(lifecycleId = this._lifecycleId) {
        const c = this.config;
    ...
    _initToggles() {
        ...
        document.querySelectorAll(selector).forEach((checkbox) => {
            ...
            this._listen(checkbox, 'change', () => {
                this.settingsStore.set(key, checkbox.checked);
                this.render();
            });
            this._ownCleanup(this.settingsStore.subscribe(key, (v) => { checkbox.checked = !!v; }));
        });
    }
    ...
    _initColorPickers() {
        ...
        for (const def of defs) {
            ...
            this._ownCleanup(this.settingsStore.subscribe(def.setting, (hex) => {
    ...
    _initHistoryAndPresets() {
        ...
        if (this.config.presets) {
            this.presetStore = new PresetStore({
                storageKey: this.config.presets.storageKey,
    ...
    _initChangeTracking() {
        this._ownCleanup(this.settingsStore.subscribe('*', (newVal, oldVal, key) => {
    ...
    _bindDragTransactions() {
        ...
        document.querySelectorAll('input[type="range"]').forEach((el) => {
            this._listen(el, 'mousedown', begin);
            this._listen(el, 'touchstart', begin, { passive: true });
        });
        document.querySelectorAll('.range-slider-thumb').forEach((el) => {
            this._listen(el, 'mousedown', begin);
            this._listen(el, 'touchstart', begin, { passive: true });
        });
        this._listen(document, 'mouseup', end);
        this._listen(document, 'touchend', end);
    }
    ...
    async exportPNG(filename, scale = 2) {
        return this.runExport('png', async ({ signal }) => {
            ...
            if (this.target.type === 'canvas') {
                ...
                await this._downloadCanvasPNG(off, name, signal);
            } else {
    ...
    _initPresetChrome() {
        ...
        if (toggle && menu) {
            ...
            this._ownCleanup(() => menu.replaceChildren?.());
        }
        const saveBtn = document.getElementById(this.config.dom?.saveBtn || 'savePresetBtn');
        if (saveBtn) this._listen(saveBtn, 'click', () => { void this.savePreset(); });
        const shareBtn = document.getElementById(this.config.dom?.shareBtn || 'presetToolbarShareBtn');
        if (shareBtn) this._listen(shareBtn, 'click', () => { void this._handleShareClick(shareBtn); });
    }
    ...
    async _handleShareClick(btn) {
        ...
        if (url && btn) {
            ...
            this._ownTimeout(() => btn.setAttribute('title', prev || 'Copy share link'), 1500);
        }
    }
    ...
    /** Transient status toast owned by the framework chrome. */
    _showToast(message) {
        let el = document.getElementById('presetShareToast');
        if (!el) {
            el = document.createElement('div');
            el.id = 'presetShareToast';
            el.className = 'ui-garage-share-toast';
            el.setAttribute('role', 'status');
            (document.querySelector('.top-links') || document.body).appendChild(el);
        }
        el.textContent = message;
        el.hidden = false;
        el.classList.add('ui-garage-share-toast--visible');
        if (this._toastTimer) this._clearOwnedTimeout(this._toastTimer);
        this._toastTimer = this._ownTimeout(() => {
            this._toastTimer = null;
            el.classList.remove('ui-garage-share-toast--visible');
            el.hidden = true;
        }, 2200);
    }
    ...
    _listen(target, type, listener, options) {
        ...
        this._ownCleanup(() => target.removeEventListener?.(type, listener, options));
        return listener;
    }

    _ownTimeout(callback, delay) {
        const timeoutId = setTimeout(() => {
            this._ownedTimeouts.delete(timeoutId);
            callback();
        }, delay);
        this._ownedTimeouts.add(timeoutId);
        return timeoutId;
    }

    _clearOwnedTimeout(timeoutId) {
    ...
    _releaseObjectUrl(url) {
        if (!this._ownedObjectUrls.delete(url)) return;
        ...
        return new Promise((resolve, reject) => {
            ...
            try {
                canvas.toBlob((blob) => {
                    ...
                    else reject(new Error('The export produced an empty file.'));
                }, 'image/png');
            } catch (error) {
                signal?.removeEventListener?.('abort', onAbort);
                reject(error);
    ...
    _downloadBlob(blob, filename) {
        ...
        try {
            ...
            a.remove();
            this._ownTimeout(() => this._releaseObjectUrl(url), 1000);
        }
    }
    ...
    async _downloadCanvasPNG(canvas, filename, signal) {
    ...
    async _rasterizeSVG(svgString, w, h, filename, signal) {
        ...
        try {
            await new Promise((resolve, reject) => {
                ...
                img.onerror = () => { cleanup(); reject(new Error('Could not render SVG for PNG export.')); };
                signal?.addEventListener('abort', onAbort, { once: true });
                this._throwIfAborted(signal);
                img.src = url;
            });
            ...
            await this._downloadCanvasPNG(canvas, filename, signal);
        } finally {
    ...
    destroy() {
        ...
        for (const timeoutId of this._ownedTimeouts) clearTimeout(timeoutId);
        ...
        for (const resource of destroyables) {
            if (!resource || destroyed.has(resource) || typeof resource.destroy !== 'function') continue;
            destroyed.add(resource);
            resource.destroy();
        }

        for (const url of this._ownedObjectUrls) URL.revokeObjectURL(url);
        this._ownedObjectUrls.clear();

        if (wasActive && typeof this.config.onDestroy === 'function') this.config.onDestroy(this);
