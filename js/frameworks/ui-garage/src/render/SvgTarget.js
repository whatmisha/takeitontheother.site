...
export class SvgTarget extends RenderTarget {
    ...
    initZoom(options = {}) {
        ...
        this._onZoomChangeEvent = () => {
            this._emitZoomChange(this.zoomPan.getZoomPercent());
        };
        this.container.addEventListener('zoomchange', this._onZoomChangeEvent);
    ...
    destroy() {
        super.destroy();
        if (this._onZoomChangeEvent) {
            this.container.removeEventListener('zoomchange', this._onZoomChangeEvent);
            this._onZoomChangeEvent = null;
        }
