        ...
        <aside id="settingsPanel" class="controls-panel" aria-label="Generator settings">
            ...
            <div class="panel-content">
                ...
                <div class="pill-toggle-row">
                    <label class="pill-toggle"><input class="sr-only" type="checkbox" data-setting="invert"><span>Invert</span></label>
                </div>
            </div>
        </aside>
        <nav class="bottom-buttons action-dock" role="toolbar" aria-label="Export actions">
            <div class="action-dock__slot action-dock__slot--utility">
                ${renderer === 'svg' ? '<button id="pngExport" class="btn-fixed" type="button">PNG</button>' : ''}
            </div>
            <div class="action-dock__slot action-dock__slot--primary">
                ${renderer === 'svg' ? '<button id="svgExport" class="btn-fixed" type="button">SVG ⌘E</button>' : '<button id="pngExport" class="btn-fixed" type="button">PNG ⌘E</button>'}
            </div>
        </nav>
    </main>
...
function appTemplate({ id, name, renderer, frameworkPath }) {
    ...
    return `import { ToolUiController, defineTool } from ${JSON.stringify(`${frameworkPath}/src/index.js`)};
...
await app.init();
const toolUi = new ToolUiController({
    id: ${JSON.stringify(id)},
    title: ${JSON.stringify(name)},
    actions: ${renderer === 'svg' ? `[
        { id: 'png', button: 'pngExport', label: 'PNG', kind: 'export', group: 'extra', run: () => app.exportPNG(${JSON.stringify(`${id}.png`)}) },
        { id: 'svg', button: 'svgExport', label: 'SVG', kind: 'export', group: 'primary', shortcut: 'mod+e', run: () => app.exportSVG(${JSON.stringify(`${id}.svg`)}) }
    ]` : `[
        { id: 'png', button: 'pngExport', label: 'PNG', kind: 'export', group: 'primary', shortcut: 'mod+e', run: () => app.exportPNG(${JSON.stringify(`${id}.png`)}) }
    ]`}
}).init();
window.addEventListener('pagehide', () => { toolUi.destroy(); app.destroy(); }, { once: true });
`;
}
