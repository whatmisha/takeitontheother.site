...
export class NuevoGenerator {
  ...
  generate({ image, width, height, color, settings }) {
    ...
    const factory = new StrokeFactory({ source: "nuevo", groupId, color, seed });
    const lineStrength = normalizePercent(settings.lineStrength, 74);
    const massOutline = normalizePercent(settings.massOutline, 68);
    const interiorDetail = normalizePercent(settings.interiorDetail, 42);
    const fill = normalizePercent(settings.fill, 24);
    const simplify = normalizePercent(settings.simplify, 34);
    const spacing = normalizeRange(settings.spacing, 16, 4, 80);
    const dotSize = normalizeRange(settings.dotSize, 18, 3, 120);
    const sizeVariation = normalizePercent(settings.sizeVariation, 0);
    const jitter = normalizePercent(settings.jitter, 18);
    const maxPoints = Math.round(normalizeRange(settings.maxPoints, 1400, 10, 25000));
