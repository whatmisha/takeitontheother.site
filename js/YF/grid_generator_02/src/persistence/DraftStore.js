const DATABASE_NAME = 'upgrade-pizza-boxer-02-v1';
...
export class DraftStore {
    ...
    async importIfEmpty(draft) {
        ...
        try {
            ...
            let result = null;
            const lookup = records.get(CURRENT_DRAFT_ID);
            lookup.addEventListener('success', () => {
                result = lookup.result || { ...draft, id: CURRENT_DRAFT_ID };
                if (!lookup.result) records.put(result);
            }, { once: true });
            await committed;
            return result;
        } catch (error) {
            console.warn('Previous draft could not be imported:', error);
            return null;
        }
    }
