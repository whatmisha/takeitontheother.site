...
} from './src/geometry/characterGeometry.js?v=20260827-2';
...
import { AnimationExporter } from './src/export/animationExporter.js?v=20260827-3';
import {
    advanceEyeMotion,
...
const settings = {
    ...
    rayLengthVariation: DEFAULT_GEOMETRY.rayLengthVariation,
    rayWidthVariation: DEFAULT_GEOMETRY.rayWidthVariation,
    rayModulationFrequency: DEFAULT_GEOMETRY.rayModulationFrequency,
    rayModulationPhase: DEFAULT_GEOMETRY.rayModulationPhase,
...
function normalizeIncomingState(source = {}) {
    ...
    normalized.rayLengthVariation = normalizeRayVariation(normalized.rayLengthVariation);
    normalized.rayWidthVariation = normalizeRayVariation(normalized.rayWidthVariation);
    normalized.rayModulationFrequency = normalizeRayFrequency(normalized.rayModulationFrequency);
    normalized.rayModulationPhase = normalizeRayPhase(normalized.rayModulationPhase);
    normalized.rayMotion = normalizeRayMotion(normalized.rayMotion);
...
function activeRenderSettings(current) {
    ...
    if (current.focusMode === 'animate') {
        const animated = settingsAtRayMotionTime(
            current,
            focusAnimation.elapsedMs,
            focusAnimation.timeline?.durationMs
        );
        if (!focusAnimation.currentFocus) return animated;
        const focus = constrainFocusPoint(focusAnimation.currentFocus, animated);
        return {
            ...animated,
            focusX: focus.x,
            focusY: focus.y
...
function previewMotionBlurGhosts(state, sourceState = state) {
    ...
    return descriptors.flatMap(({ offsetFrames, opacity }) => {
        ...
        const timedState = settingsAtRayMotionTime(
            sourceState,
            time,
            focusAnimation.timeline.durationMs
        );
...
const app = defineTool({
    ...
    controls: {
        sliders: [
            ...
            { id: 'rayLengthVariationSlider', valueId: 'rayLengthVariationValue', setting: 'rayLengthVariation', min: 0, max: 100, decimals: 0, baseStep: 1, shiftStep: 10 },
            { id: 'rayWidthVariationSlider', valueId: 'rayWidthVariationValue', setting: 'rayWidthVariation', min: 0, max: 100, decimals: 0, baseStep: 1, shiftStep: 10 },
            { id: 'rayModulationFrequencySlider', valueId: 'rayModulationFrequencyValue', setting: 'rayModulationFrequency', min: 1, max: 6, decimals: 0, baseStep: 1, shiftStep: 1 },
            { id: 'rayModulationPhaseSlider', valueId: 'rayModulationPhaseValue', setting: 'rayModulationPhase', min: 0, max: 360, decimals: 0, baseStep: 1, shiftStep: 10 },
            ...
            { id: 'rayMotionSlider', valueId: 'rayMotionValue', setting: 'rayMotion', min: 0, max: 4, decimals: 0, baseStep: 1, shiftStep: 1 },
    ...
    share: {
        ...
        quantizableFloatKeys: [
            ...
            'rayLengthVariation', 'rayWidthVariation', 'rayModulationFrequency', 'rayModulationPhase',
            ...
            'motionDuration', 'motionPointCount', 'motionComplexity', 'motionSmoothness', 'motionStopCount', 'motionSpeedVariation', 'rayMotion',
