...
test('ray motion changes harmonic phase and returns exactly at the loop boundary', () => {
    const animatedSettings = {
        ...settings,
        motionDuration: 4,
        rayLengthVariation: 100,
        rayWidthVariation: 75,
        rayModulationFrequency: 1,
        rayModulationPhase: 20,
        rayMotion: 1
    };
    ...
    const quarter = buildAnimationFrameScene(animatedSettings, focus, {
        timeMs: 1000,
        durationMs: 4000
    });
    ...
    assert.notEqual(quarter.character.rounded.path, start.character.rounded.path);
