...
- `Stops` задаёт точное число остановок от 1 до `Points`; стартовая точка входит в число и всегда активна. Остальные Stop-точки выбираются детерминированно по seed.
- Hold budget растёт на 4% общей длительности на каждую остановку, но не выше 60%, и равномерно делится между ними.
...
- При `Stops = 1` Ease in–out применяется один раз ко всей замкнутой кривой; при большем значении — один раз на каждый непрерывный участок между выбранными Stop-точками. Промежуточные anchors проходятся без замедления.
...
Сохраняемые настройки анимации: `focusMode`, `motionDuration`, `motionPointCount`, `motionComplexity`, `motionSmoothness`, `motionStopCount`, `motionSpeedVariation`, `motionBlinkCount`, `motionEmotionVariation`, `motionBlur`, `motionSeed` и `showMotionPath`. `Stops` — точное целое количество остановок от 1 до `Points`, причём стартовая точка всегда входит в это число. Ease in–out применяется один раз между активными остановками, поэтому промежуточные геометрические точки проходятся без ложного торможения. `Speed variation` детерминированно перераспределяет неизменный бюджет движения между Bézier-сегментами: 0% сохраняет скорость по длине пути, 100% даёт максимальный контраст скоростей от точки до точки. Технические параметры экспорта удалены из пользовательского state.
...
- внутренний pause budget равномерно делится между точным числом активных остановок;
- movement budget делится пропорционально длинам сегментов;
- easing применяется к arc-length progress сегмента;
- выбор Stop-точек воспроизводим по seed и не меняет path;
...
`src/animation/eyeTimeline.js` размещает запрошенные моргания внутри активных hold-интервалов. На одну точку назначается не более двух морганий, поэтому фактическое количество ограничено числом доступных остановок × 2.
...
- unit tests timeline, Ease in–out, точное количество Stops, pause budget и детерминированный выбор точек;
