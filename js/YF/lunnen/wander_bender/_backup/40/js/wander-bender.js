...
class WanderBenderGenerator {
    ...
    // line can be: [{x, y}, {x, y}] for straight, or [{x, y}, {x, y}, arcAmount] for arc
    // distance is the distance along the line from the start
    getPointOnLine(line, distance) {
    ...
    // line can be: [{x, y}, {x, y}] for straight, or [{x, y}, {x, y}, arcAmount] for arc
    // width is the thickness of the resulting shape
    // cornerRadius is applied to the ends
    // pivotDistance is the distance along the line where the pivot point should be
    createShapeFromLine(line, width, cornerRadius, pivotDistance) {
        ...
        const pivotPoint = this.getPointOnLine(line, pivotDistance);
    ...
    renderRadialComposition(stroke) {
        ...
        for (let i = 0; i < this.radialRays.length; i++) {
            ...
            const result = this.createShapeFromLine(line, width, cornerRadius, pivotDistance);
            if (!result) continue;
        ...
        for (let i = 0; i < this.radialRays.length; i++) {
            ...
            hitArea.dataset.rayIndex = i;
            
            // Add click handler
            hitArea.addEventListener('click', (e) => {
                e.stopPropagation();
                this.extractRadialRay(i);
            });
        ...
        for (const index of this.extractedRadialIndices) {
            ...
            path.dataset.rayIndex = index;
            path.style.cursor = 'pointer';
            
            // Add click handler to return to united group
            path.addEventListener('click', (e) => {
                e.stopPropagation();
                this.returnRadialRay(index);
            });
    ...
    renderRandomComposition(stroke, areaWidth, areaHeight) {
        ...
        areaRect.setAttribute('x', this.centerX - areaWidth / 2);
        areaRect.setAttribute('y', this.centerY - areaHeight / 2);
        ...
        this.group.appendChild(areaRect);
        ...
        for (let i = 0; i < this.randomRectangles.length; i++) {
            ...
            hitArea.style.cursor = 'pointer';
            hitArea.classList.add('hit-area');
            hitArea.dataset.rectIndex = i;
            
            // Add click handler
            hitArea.addEventListener('click', (e) => {
                e.stopPropagation();
                this.extractRectangle(i);
            });
        ...
        for (const index of this.extractedIndices) {
            ...
            path.setAttribute('stroke-linejoin', 'round');
            path.classList.add('extracted-shape');
            path.dataset.rectIndex = index;
            path.style.cursor = 'pointer';
            
            // Add click handler to return to united group
            path.addEventListener('click', (e) => {
                e.stopPropagation();
                this.returnRectangle(index);
            });
    ...
    generateFlowField(params) {
        ...
        console.log(`Flow Field: placed ${successfulPlacements} elements in ${attempts} attempts`);
    ...
    renderFlowFieldComposition(stroke, areaWidth, areaHeight) {
        ...
        for (let i = 0; i < this.flowElements.length; i++) {
            ...
            const result = this.createShapeFromLine(
                el.line,
                el.width,
                el.cornerRadius,
                el.pivotDistance
        ...
        if (unitedPath) {
            ...
            svgPath.classList.add('united-shape');
            
            this.group.appendChild(svgPath);
        }
        ...
        for (let i = 0; i < this.flowElements.length; i++) {
            ...
            hitArea.style.cursor = 'pointer';
            hitArea.addEventListener('click', () => this.extractFlowElement(i));
            this.group.appendChild(hitArea);
        }
        ...
        for (const index of this.extractedFlowIndices) {
            ...
            const extractedPath = document.createElementNS('http://www.w3.org/2000/svg', 'path');
            extractedPath.setAttribute('d', pathData);
            extractedPath.setAttribute('fill', '#000000');
            extractedPath.setAttribute('stroke', '#FFFFFF');
            extractedPath.setAttribute('stroke-width', strokeWidth);
            extractedPath.setAttribute('stroke-linecap', 'round');
            extractedPath.setAttribute('stroke-linejoin', 'round');
            extractedPath.setAttribute('class', 'extracted-shape');
            extractedPath.style.cursor = 'pointer';
            extractedPath.addEventListener('click', () => this.returnFlowElement(index));
            this.group.appendChild(extractedPath);
        }
    }
...
function debouncedUpdate(immediate = false) {
    if (immediate) {
