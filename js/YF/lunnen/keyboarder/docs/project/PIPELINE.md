...
Все числовые значения приведены на примере проекта **LCAKB23** (`reference/lcakb23/LCAKB23.svg`) — 110 клавиш,
...
```json
{
  "meta": { "name": "LCAKB23", "source": "reference/lcakb23/LCAKB23.svg", "unit": "px" },
...
Числа `origin` и `blocks` выше относятся к более раннему сохранению файла. Текущий `reference/lcakb23/LCAKB23.svg`
...
Распознавание
   ├─ отрезки blueprint → корзины H/V, диагонали отбросить    (раздел 4.1)
...
Генерация
   ├─ caps:   чейнинг по сетке + close/fillToColumn/rowSpan    (раздел 5)
...
| `final.py` | типизация по шаблонам, выгрузка `reference/lcakb23/LCAKB23.layout.json` (§ 11.2) |
| `zoom.py` | диагностические рендеры фрагментов с оверлеями поля, ink-бокса и capline |

Артефакт: `reference/lcakb23/LCAKB23.layout.json` — таблица 110 клавиш с полями `row`, `x`, `w`, `h`, `u`, `block`,
