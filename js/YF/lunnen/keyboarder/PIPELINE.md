...
Все числовые значения приведены на примере проекта **LCAKB23** (`LCAKB23.svg`) — 110 клавиш,
...
![Раскладка LCAKB23](docs/overview.png)
...
![Слоты в алфавитном блоке](docs/slots-alpha.png)
![Слоты в F-ряду](docs/slots-frow.png)
![Слоты в нампаде](docs/slots-numpad.png)
![Иконки](docs/icons.png)
...
```json
{
  "meta": { "name": "LCAKB23", "source": "LCAKB23.svg", "unit": "px" },
...
Числа `origin` и `blocks` выше относятся к более раннему сохранению файла. Текущий `LCAKB23.svg`
...
| `final.py` | типизация по шаблонам, выгрузка `LCAKB23.layout.json` (§ 11.2) |
| `zoom.py` | диагностические рендеры фрагментов с оверлеями поля, ink-бокса и capline |

Артефакт: `LCAKB23.layout.json` — таблица 110 клавиш с полями `row`, `x`, `w`, `h`, `u`, `block`,
