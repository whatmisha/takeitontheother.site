...
`editId`, поэтому легенды, ширины и content overrides едут вместе с клавишей.
`Reset` удаляет overrides выбранных клавиш/активной ширины и возвращает контент или геометрию из
генератора.

Осталось по этапу 4: добавление рядов, удаление/восстановление рядов, richer JSON-модель для
полного кругового цикла и явная edited-keyboard модель поверх текущих preset override blobs.
