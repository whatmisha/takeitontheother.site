# Keyboard Layout Studio — план реализации
...
Инструмент работает **в px как во внутренней единице** (совпадает с SVG и с pt), а мм показывает
рядом в интерфейсе. Так экспорт в PDF получает физически точный размер страницы бесплатно:
`SVGExporter.exportToPDF` умеет `unit: 'mm'`, и наш px переводится в мм ровно.
...
│       ├── fontprobe.js       ← полный съём метрик любой гарнитуры (этап 8)
...
Статус: **этапы 0 и 1 сделаны**, дальше по порядку.
...
- панель **Grid**: шаг X, шаг Y, ширина 1U, высота, радиус, инсет — шесть **независимых**
  слайдеров (решение 1) с двойным выводом px и мм;
...
- панель **Type**: кегли по ролям, интерлиньяж, трекинг, режим компенсации
  (`выкл` / `модель` / `ручная таблица`);
- панель **Legend**: инспектор выбранной клавиши — шаблон, содержимое слотов, вычисленная
  компенсация по знакам, ручной оверрайд;
...
### Этап 3 — бета, верификация (1 день)
...
### Этап 4 — RC, редактирование (2–3 дня)
...
### Этап 8 — про, любая гарнитура (4–5 дней)

Сейчас модель компенсации привязана к `YS Text Regular`: коэффициенты 41.34 / −42.60 и параметры
окна `eps = 32`, `w = 300` получены МНК на 69 знаках именно этого шрифта. Цель этапа — **загрузить
произвольный шрифтовой файл, вытащить из него все метрики и выключить легенды без единой ручной
правки**.

#### 8.1. Полный съём метрик из файла

`kb/fontprobe.js` — разбор всего, что понадобится, с честными фолбэками. Многие шрифты врут
в `OS/2`, поэтому геометрическое измерение всегда приоритетнее объявленного.
...
- выбор нескольких гарнитур в одном макете (знак одной, слова другой);
...
| 8. Про: любая гарнитура | 4–5 дней |
...
уйдёт на подбор нормировок, а не на реализацию.
...
| 5 | Шрифты | **Не копировать**, читать `Fonts/YS Text/YS Text-Regular.ttf` напрямую. Только Regular |
...
**3. Название.** Условно Keyboard Layout Studio. От него зависят `storageKey` пресетов
и имена экспортируемых файлов.

**4. Разрывы между блоками.** Сейчас 11.88 px (1.61 обычного зазора) между `main` / `nav` /
`numpad`. Это подгонка под конкретный корпус или параметр, который пользователь должен менять?

**5. Лицензия шрифта.** У вариативной версии `fsType=4` (Preview & Print). Для локальной работы
вопрос не стоит; если инструмент когда-нибудь выложим в сеть, TTF будет качаться браузером
в открытом виде.

**6. Набор раскладок для этапа 5.** Какие форм-факторы нужны кроме нашего 96 % — TKL, ISO,
