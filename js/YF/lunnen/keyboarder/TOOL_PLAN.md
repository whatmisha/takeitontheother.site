...
`Shift`/`Cmd`/`Ctrl`-клик для мультивыбора, синхронизация с Legend inspector, навигация
...
клавишам. `Reset` удаляет overrides выбранных клавиш и возвращает контент из генератора.

Осталось по этапу 4: изменение ширин с пересчётом ряда, добавление/удаление клавиш и рядов,
ручные оверрайды компенсации, richer JSON-модель для полного кругового цикла и добавление/удаление
самих legend elements сверх существующих template-вариантов.
