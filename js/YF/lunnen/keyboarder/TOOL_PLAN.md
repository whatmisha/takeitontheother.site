...
`#selection` временно скрывается на время SVG/PNG-экспорта и возвращается после него.

Осталось по этапу 4: массовая смена шаблона, правка содержимого слотов, изменение ширин с
пересчётом ряда, добавление/удаление клавиш и рядов, ручные оверрайды компенсации и сохранение
этих изменений в модель.
