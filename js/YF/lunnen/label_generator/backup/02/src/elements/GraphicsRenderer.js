...
export class GraphicsRenderer {
    ...
    scaleSVG(svgGroup, targetWidth, targetHeight, originalWidth, originalHeight) {
        ...
        // Если это SVG элемент и размеры уже совпадают (штрихкод генерируется в финальных размерах)
        if (svgGroup.tagName === 'svg') {
            // Проверяем, нужно ли масштабирование
            const currentWidth = parseFloat(svgGroup.getAttribute('width'));
            const currentHeight = parseFloat(svgGroup.getAttribute('height'));
            
            // Если размеры уже совпадают (с небольшой погрешностью), ничего не делаем
            if (Math.abs(currentWidth - targetWidth) < 0.01 && Math.abs(currentHeight - targetHeight) < 0.01) {
                return svgGroup;
            }
            
            // Иначе масштабируем через width/height
            svgGroup.setAttribute('width', targetWidth);
            svgGroup.setAttribute('height', targetHeight);
