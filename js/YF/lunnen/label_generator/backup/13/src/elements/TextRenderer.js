...
export class TextRenderer {
     ...
     * ВНИМАНИЕ: размеры кегля и интерлиньяжа теперь хранятся в пунктах (pt) в Settings.
     * Здесь мы конвертируем их в мм через MathUtils.ptToMm, чтобы работать в единицах SVG.
     */
    renderWrappedText(text, maxWidth, style, scale) {
        ...
        // lineHeight в стиле хранится в pt -> конвертируем в мм.
        let lineHeightPt = style.lineHeight;
        if (!lineHeightPt || isNaN(lineHeightPt) || lineHeightPt <= 0) {
            // Безопасный дефолт: 1.3 * кегль (в pt)
            lineHeightPt = (style.size && !isNaN(style.size) && style.size > 0)
                ? style.size * 1.3
                : 16;
        }
        const lineHeight = MathUtils.ptToMm(lineHeightPt);
     ...
     * style.size хранится в пунктах (pt) -> конвертируем в мм через calculateFontSize().
     */
    createTextElement(content, style, scale, alignment = 'left') {
        // Вычисляем размер шрифта (в mm)
    ...
    /**
     * Расчет размера шрифта на основе EM-кегля в пунктах (pt).
     * style.size — это EM-кегль в pt, который прямо переводится в mm userUnits SVG.
     * Это обеспечивает совпадение значения в панели и в поле font-size Illustrator.
     */
    calculateFontSize(style) {
        // EM-кегль в pt (из настроек)
        let sizePt = style.size;
        if (!sizePt || isNaN(sizePt) || sizePt <= 0) {
            // Безопасный дефолт, если настройки повреждены
            sizePt = 16;
        }
        const fontSizeMm = MathUtils.ptToMm(sizePt);
        
        // На всякий случай защищаемся от NaN/Infinity
        if (!fontSizeMm || !isFinite(fontSizeMm) || fontSizeMm <= 0) {
            return MathUtils.ptToMm(16);
        }
        
        return fontSizeMm; // в мм
    }
    ...
    getTextStyle(styleRef) {
        const styles = {
            'headline': {
                ...
                // headlineSize и lineHeight теперь в pt
                size: this.settings.get('headlineSize') || 21.5,
                lineHeight: this.settings.get('lineHeight') || 28.6,
            ...
            'text': {
                ...
                size: this.settings.get('textSize') || 7.2,
                lineHeight: this.settings.get('textLineHeight') || 14.3,
            ...
            'lunnenDisplay': {
                ...
                fontWeight: 400,
                size: this.settings.get('lunnenDisplaySize') || 43.0,
                lineHeight: this.settings.get('lunnenDisplayLineHeight') || 57.3,
