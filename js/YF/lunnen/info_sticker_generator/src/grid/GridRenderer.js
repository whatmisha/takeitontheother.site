...
export class GridRenderer {
    ...
    /**
     * Отрисовка сетки для стикера (упрощенная версия без боковых панелей)
     * @param {SVGElement} svg - SVG элемент для отрисовки
     */
    renderStickerGrid(svg) {
        ...
        // Отрисовка колонок
        if (showColumns) {
            this.drawStickerColumns(svg, width, height);
        }

        // Отрисовка baseline
        if (showBaseline) {
            this.drawStickerBaseline(svg, width, height);
        }

        // Отрисовка рамки
        this.drawStickerBorder(svg, width, height);
    }

    /**
     * Отрисовка колонок для стикера
     */
    drawStickerColumns(container, width, height) {
        const module = this.settings.get('gridModule');
        const margins = this.settings.get('margins');
        ...
        const availableWidth = width - 2 * margins;
        const totalGutters = (columnCount - 1) * module;
        const columnWidth = (availableWidth - totalGutters) / columnCount;
        ...
        for (let i = 0; i < columnCount; i++) {
            ...
            currentX += columnWidth + module;
        }
    }

    /**
     * Отрисовка baseline для стикера
     */
    drawStickerBaseline(container, width, height) {
        ...
        const endY = height - margins;
        const startX = margins;
        const endX = width - margins;
