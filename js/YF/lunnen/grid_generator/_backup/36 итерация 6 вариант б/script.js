...
class GridGenerator {
    ...
    */
    ...
    showIconsPanel() {
        ...
        const block = this.iconsBlock;
        if (!block) {
            console.warn('Icons block not found');
            return;
        }
    ...
    // Show claim settings panel
    showClaimPanel() {
        if (!this.dom.claimPanel) return;
        
        const block = this.claimBlock;
        if (!block) {
            console.warn('Claim block not found');
            return;
        }
        ...
        // Закрываем панель иконок, если она открыта
        if (this.dom.iconsPanel && this.dom.iconsPanel.style.display !== 'none') {
            this.closeIconsPanel();
        }
        
        // Если панель открывается впервые или была закрыта, центрируем её
        if (!this.claimPanel || !this.claimPanel.wasOpened) {
            const panel = this.dom.claimPanel;
            const rect = panel.getBoundingClientRect();
            const viewportWidth = window.innerWidth;
            const viewportHeight = window.innerHeight;
            ...
            this.claimPanel.wasOpened = true;
        }
        
        // Обновляем значения полей из текущих настроек
        
        if (this.dom.claimXInput) {
            this.dom.claimXInput.value = block.x;
        }
        if (this.dom.claimRowInput) {
        ...
        if (this.dom.claimHeightInput) {
            this.dom.claimHeightInput.value = block.heightInModules.toFixed(2);
        }
        
        // Показываем панель
        this.dom.claimPanel.style.display = 'flex';
        this.dom.claimPanel.classList.add('active');
        ...
        setTimeout(() => {
            ...
            const handleClickOutside = (e) => {
                if (!this.dom.claimPanel.contains(e.target) && 
                    !e.target.closest('#claim-group')) {
                    this.closeClaimPanel();
                    document.removeEventListener('click', handleClickOutside);
                }
            };
    ...
    // Close claim settings panel
    closeClaimPanel() {
        if (!this.dom.claimPanel) return;
        this.dom.claimPanel.style.display = 'none';
    ...
    exportSVG() {
        ...
        // ============================================
        // Итерация 6: Используем рендереры для экспорта (чистый SVG)
        // ============================================
        if (this.textRenderer && this.textBlockManager) {
            // Используем менеджеры и рендереры для экспорта
            
            // Создаем группу с трансформацией для позиционирования
            const objectsGroup = document.createElementNS('http://www.w3.org/2000/svg', 'g');
            objectsGroup.setAttribute('id', 'objects');
            objectsGroup.setAttribute('transform', `translate(${frontX}, ${frontY})`);
            exportSvg.appendChild(objectsGroup);
            
            // Рендерим текстовые блоки
            const visibleTextBlocks = this.textBlocks.filter(block => block.visible !== false);
            if (visibleTextBlocks.length > 0) {
                const textGroupContainer = document.createElementNS('http://www.w3.org/2000/svg', 'g');
                textGroupContainer.setAttribute('id', 'text-blocks');
                objectsGroup.appendChild(textGroupContainer);
                this.textRenderer.renderAll(textGroupContainer, visibleTextBlocks, scale);
            }
            
            // Рендерим графические блоки
            const visibleGraphicsBlocks = this.graphicsBlocks.filter(block => block.visible !== false);
            if (visibleGraphicsBlocks.length > 0) {
                const graphicsGroupContainer = document.createElementNS('http://www.w3.org/2000/svg', 'g');
                graphicsGroupContainer.setAttribute('id', 'graphics-blocks');
                objectsGroup.appendChild(graphicsGroupContainer);
                this.graphicsRenderer.renderAll(graphicsGroupContainer, visibleGraphicsBlocks, scale);
            }
        } else {
            // Fallback: используем старый код если рендереры недоступны
            this.textBlocks.forEach(block => {
