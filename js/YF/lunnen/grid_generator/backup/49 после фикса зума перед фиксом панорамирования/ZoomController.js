...
export class ZoomController {
    ...
    updateTransform() {
        ...
        if (this.zoom === 1.0) {
            ...
            this.canvasContainer.style.padding = '';
        } else {
            // При зуме > 100% - добавляем padding для доступа ко всем краям
            // Padding масштабируется с уровнем зума для обеспечения доступа ко всему контенту
            const paddingAmount = Math.ceil(50 * this.zoom);
            this.canvasContainer.setAttribute('data-zoomed', 'true');
            this.canvasContainer.style.padding = `${paddingAmount}vh ${paddingAmount}vw`;
        }
    }
