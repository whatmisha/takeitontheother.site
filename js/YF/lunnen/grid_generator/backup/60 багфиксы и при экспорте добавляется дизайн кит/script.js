...
class GridGenerator {
    ...
    */
    ...
    async createExportSVG() {
        ...
        // Add Lunnen logo in multiple sizes outside artboard (for reference in editor)
    ...
    /**
     * Добавить логотип Lunnen в нескольких размерах за пределами артборда
     */
    async addLunnenLogoReference(svg, artboardWidth, scale = 1) {
        try {
            // Загружаем SVG логотипа
            const response = await fetch('graphics/lunnen_logo.svg');
            if (!response.ok) {
                console.warn('Failed to load Lunnen logo');
                return;
            }
            ...
            const logoSvgElement = logoDoc.querySelector('svg');
            
            if (!logoSvgElement) {
                console.warn('Invalid logo SVG structure');
                return;
            }
            
            // Получаем оригинальные размеры из viewBox
            const viewBox = logoSvgElement.getAttribute('viewBox');
            ...
            const verticalGap = 3; // mm между логотипами
            
            // Размеры в модулях (от большего к меньшему)
            const sizes = [6, 5, 4, 3, 2, 1];
            
            // Создаем группу для всех логотипов
            const logosGroup = document.createElementNS('http://www.w3.org/2000/svg', 'g');
            logosGroup.setAttribute('id', 'lunnen-logos-reference');
            logosGroup.setAttribute('opacity', '0.7');
            ...
            sizes.forEach(heightInModules => {
                ...
                // Создаем группу для этого логотипа
                const logoGroup = document.createElementNS('http://www.w3.org/2000/svg', 'g');
                logoGroup.setAttribute('transform', `translate(${startX * scale}, ${currentY * scale}) scale(${(heightInMm / originalHeight) * scale})`);
                
                // Копируем содержимое логотипа
                const logoContent = logoSvgElement.querySelector('path, g');
                if (logoContent) {
                    const clonedContent = logoContent.cloneNode(true);
                    logoGroup.appendChild(clonedContent);
                }
                
                logosGroup.appendChild(logoGroup);
                
                // Добавляем подпись с размером справа от логотипа
                const labelX = startX + widthInMm + 5; // 5mm отступ справа от логотипа
                const labelY = currentY + (heightInMm / 2); // По центру высоты логотипа
                ...
                logosGroup.appendChild(label);
                
                // Обновляем позицию для следующего логотипа
                currentY += heightInMm + verticalGap;
            });
            
            svg.appendChild(logosGroup);
        } catch (error) {
            console.error('Error adding Lunnen logo reference:', error);
        }
    }
