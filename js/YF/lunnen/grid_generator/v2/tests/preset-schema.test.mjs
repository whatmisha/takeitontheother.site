...
test('every checked-in source-of-truth preset satisfies schema 2.0', async () => {
    const files = (await readdir(presetsUrl))
        .filter(name => name.endsWith('.json') && name !== 'manifest.json');
    assert.ok(files.length > 0);
    for (const name of files) {
        const preset = await readPreset(name);
        assert.equal(preset.version, '2.0', name);
...
test('schema rejects invalid nested editor values with useful paths', async () => {
    ...
    preset.net.planes[1].contentRotation = 45;
    ...
    assert.throws(
        ...
        error => {
            assert.match(error.message, /net\.planes\[1\]\.contentRotation/);
