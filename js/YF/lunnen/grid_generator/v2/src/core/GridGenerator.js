...
export class GridGenerator {
    ...
    /**
     * Инициализация панели плоскостей развёртки.
     */
    initSurfaceControls() {
        this.surfacePanelController = this.lifecycle.own(new SurfacePanelController({
            ...
            onPlanesRemoved: ids => this.reparentObjectsToRoot(ids),
            resolvePlaneAtPointer: (clientX, clientY) => (
                this.surfaceCoordinates.getSurfacePointer(clientX, clientY)?.surface || null
            )
        }));
    ...
    /**
     * Moves objects off planes that no longer exist.
     *
     * Deleting a plane should not delete the work that sat on it, so its
     * objects land on the root plane at their existing grid coordinates.
     */
    reparentObjectsToRoot(planeIds = []) {
        const removed = new Set(planeIds);
        if (removed.size === 0) return 0;
        const rootId = this.surfaceManager.getRootId();
        const blocks = [
            ...this.objectDocument.textBlocks,
            ...this.objectDocument.graphicsBlocks
        ].filter(block => removed.has(block.planeId));
        blocks.forEach(block => { block.planeId = rootId; });
    ...
    moveBlockToSurface(block, surface) {
        if (!block || !this.surfaceManager.has(surface)) return;
    ...
    /** The plane an object lives on, falling back to the net's root. */
    resolveBlockPlane(block) {
        return this.surfaceCoordinates.resolvePlaneId(block);
    }

    getSurfaceGridContext(surface = this.surfaceManager.getRootId()) {
        return this.surfaceCoordinates.getGridContext(surface);
    }

    // Конвертировать Row + BaselineOffset в Y (позиция в baseline модулях)
    rowBaselineToY(row, baselineOffset, surface = this.surfaceManager.getRootId()) {
    ...
    yToRowBaseline(y, surface = this.surfaceManager.getRootId()) {
    ...
    columnsToMm(columns, surface = this.surfaceManager.getRootId()) {
        return this.surfaceCoordinates.columnsToMm(columns, surface);
    }

    // Конвертировать мм в колонки
    mmToColumns(widthMm, surface = this.surfaceManager.getRootId()) {
    ...
    initTextLayout() {
        this.textLayout = new TextLayout({
            ...
            getBlockY: block => this.getBlockY(block),
            resolvePlaneId: block => this.resolveBlockPlane(block)
        });
    }
