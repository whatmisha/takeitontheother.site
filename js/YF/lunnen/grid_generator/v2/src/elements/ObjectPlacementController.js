...
export class ObjectPlacementController {
    ...
    getColumnMetrics(context) {
        ...
        const contentWidth = Math.max(
            gutter,
            context.planeWidth - 2 * context.margins * gutter
        );
    ...
    transferRelativeWidth(block, targetSurface, type) {
        const sourceSurface = this.host.resolveBlockPlane(block);
    ...
    constrain(block, type) {
        ...
        const contentHeight = Math.max(module, context.planeHeight - 2 * margins * module);
        const maxBaseline = Math.max(1, Math.floor(contentHeight / module + 1e-9));

        block.planeId = surface;
        ...
        if (type === 'text') {
            ...
            const columnWidth = Math.max(
                module * 0.1,
                (context.planeWidth - module * margins * 2 - module * (columnCount - 1)) /
                    columnCount
            );
    ...
    getPointerOffset(block, clientX, clientY) {
        ...
        if (!pointer || pointer.surface !== this.host.resolveBlockPlane(block)) return { x: 0, y: 0 };
    ...
    positionAtPointer(block, clientX, clientY, type, offset = { x: 0, y: 0 }) {
        ...
        const columnWidth = Math.max(
            module * 0.1,
            (context.planeWidth - 2 * margin - (context.columnCount - 1) * module) /
                context.columnCount
        );
    ...
    /**
     * Pins built-in graphics to the bottom of the plane they sit on.
     *
     * The last usable baseline comes from that plane's own grid, so a built-in
     * moved onto a wall lands at the wall's bottom rather than the root's.
     */
    updateBuiltInPositions() {
        this.host.objectDocument.getBuiltInGraphicsBlocks().forEach(block => {
            const surface = this.host.resolveBlockPlane(block);
            const context = this.host.surfaceCoordinates.getGridContext(surface);
            const contentHeight = context.planeHeight - context.margins * 2 * context.gridModule;
            const lastModule = Math.floor(contentHeight / context.gridModule);
