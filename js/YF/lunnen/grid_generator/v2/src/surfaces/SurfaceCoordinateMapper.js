...
export class SurfaceCoordinateMapper {
    ...
    /** The plane an object lives on; objects with none belong to the root. */
    resolvePlaneId(block) {
        const planeId = block?.planeId;
        return typeof planeId === 'string' && this.surfaceManager.has(planeId)
            ? planeId
            : this.surfaceManager.getRootId();
    }

    getGridContext(surface = this.surfaceManager.getRootId()) {
        const layout = {
            x: 0,
            y: 0,
    ...
    getBlockY(block) {
        return this.rowBaselineToY(
            ...
            this.resolvePlaneId(block)
        );
    }

    getColumnMetrics(surface = this.surfaceManager.getRootId()) {
