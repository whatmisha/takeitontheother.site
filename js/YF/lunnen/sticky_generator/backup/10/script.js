    ...
    */
    ...
    convertSheetUrlToCsv(url, sheetNumber = 1) {
        ...
        if (url.includes('/pub?output=csv') || url.includes('/export?format=csv')) {
            // Если в URL уже есть gid, оставляем его
            if (url.includes('&gid=') || url.includes('?gid=')) {
                return url;
            }
            // Если gid нет, добавляем его
            const separator = url.includes('?') ? '&' : '?';
            // Для опубликованных таблиц используем номер листа как gid (может не всегда работать)
            return `${url}${separator}gid=${sheetNumber - 1}`;
        }

        // Проверяем формат опубликованного URL (без output=csv)
        const pubMatch = url.match(/\/spreadsheets\/d\/e\/([a-zA-Z0-9-_]+)/);
        if (pubMatch) {
            return `https://docs.google.com/spreadsheets/d/e/${pubMatch[1]}/pub?output=csv&gid=${sheetNumber - 1}`;
        }
        ...
        let gid = sheetNumber - 1; // По умолчанию используем номер листа (начиная с 0)
        
        const gidMatch = url.match(/[#&]gid=(\d+)/);
        if (gidMatch) {
            // Если gid найден в URL, используем его
            gid = gidMatch[1];
        }
    ...
    async loadDataFromGoogleSheets() {
        ...
        try {
            // Получаем номер листа из поля ввода
            const sheetNumber = this.dom.sheetNumber ? 
                parseInt(this.dom.sheetNumber.value) || 1 : 1;
            
            // Конвертируем URL в CSV формат
            const csvUrl = this.convertSheetUrlToCsv(url, sheetNumber);
