...
const expectedLiterals = new Map([
    ...
    ['infra/framework/demo/tool.js', ['upgrade:framework-demo:presets:v1']],
    ['infra/framework/src/preset/PresetStore.js', ['upgrade:framework:presets:v1']],
    ['infra/framework/src/core/ApplicationShell.js', ['upgrade:framework:presets:v1']],
...
for (const [relativePath, text] of checkedText) {
    for (const original of originalNamespaces) {
        if (original === 'lunnen-grid-generator' && relativePath.startsWith('grid_generator/runtime/assets/')) {
            // Production promotion permits exactly one read-only legacy draft
            // source, never changing the active database or clearing old data.
            assert.equal(text.split(original).length - 1, 1, 'Only the reviewed legacy reader may name the old database');
            const reader = await readFile(path.join(upgradeRoot, 'grid_generator/src/persistence/LegacyDraftMigration.js'), 'utf8');
            assert.match(reader, /transaction\('drafts', 'readonly'\)/u);
            assert.doesNotMatch(reader, /['"]readwrite['"]|deleteDatabase|createObjectStore|\.put\(/u);
            continue;
        }
...
const presetStoreUrl = pathToFileURL(path.join(
    upgradeRoot,
    'infra/framework/src/preset/PresetStore.js'
)).href;
