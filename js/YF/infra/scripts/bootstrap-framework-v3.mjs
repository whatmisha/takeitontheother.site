...
const yfRoot = path.basename(upgradeRoot) === 'YF' ? upgradeRoot : path.dirname(upgradeRoot);
const siteRoot = path.resolve(yfRoot, '..', '..');
const sourceRoot = path.join(siteRoot, 'js', 'othersite-ui-framework', 'v3');
const upstreamRoot = path.join(upgradeRoot, 'infra/framework', 'upstream-v3');
const workingRoot = path.join(upgradeRoot, 'infra/framework');
...
const manifest = JSON.parse(await readFile(path.join(upgradeRoot, 'infra/SOURCE_MANIFEST.json'), 'utf8'));
...
if (checkOnly) {
    if (await readFile(provenancePath, 'utf8') !== serialized) throw new Error('infra/framework/UPSTREAM_V3.json is stale');
