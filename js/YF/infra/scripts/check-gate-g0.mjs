...
const requiredFiles = [
    ...
    'infra/docs/MIGRATION_STATUS.md',
    'infra/SOURCE_MANIFEST.json',
    'infra/baselines/TEST_BASELINES.md',
    'infra/baselines/VISUAL_BASELINES.md',
    'infra/baselines/RUNTIME_BASELINES.md',
    'infra/baselines/BROWSER_CAPTURE.json',
    'infra/baselines/donor-notes/grid-v2.md',
    'infra/baselines/donor-notes/wander-pattern.md',
    'infra/baselines/donor-notes/void.md'
];
...
try {
    sourceManifest = JSON.parse(await readFile(path.join(upgradeRoot, 'infra/SOURCE_MANIFEST.json'), 'utf8'));
} catch (error) {
    failures.push(`SOURCE_MANIFEST.json cannot be parsed: ${error.message}`);
}

try {
    browserCapture = JSON.parse(await readFile(path.join(upgradeRoot, 'infra/baselines/BROWSER_CAPTURE.json'), 'utf8'));
} catch (error) {
    failures.push(`BROWSER_CAPTURE.json cannot be parsed: ${error.message}`);
}

try {
    testBaseline = await readFile(path.join(upgradeRoot, 'infra/baselines/TEST_BASELINES.md'), 'utf8');
} catch {
...
if (browserCapture) {
    ...
    for (const filename of expectedScreenshots) {
        ...
        const screenshotPath = path.join(upgradeRoot, 'infra/baselines', 'screenshots', filename);
        try {
