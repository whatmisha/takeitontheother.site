...
assert.ok(args[0] === '--verify-originals' && (args.length === 1 || args.length === 3 && args[1] === '--source-root'), 'Use --verify-originals --source-root <archived YF directory>');
const baseline = JSON.parse(await readUpgrade('infra/qa/migrations/SOURCE_BASELINE.json'));
assert.ok(args[2], 'Original YF was archived during promotion. Pass --source-root with the backup directory; never compare redirect pages to old sources.');
const yfRoot = await realpath(args[2]);
