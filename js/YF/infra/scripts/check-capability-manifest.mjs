...
const manifest = JSON.parse(await readFile(new URL('infra/APPLICATION_CAPABILITIES.json', root), 'utf8'));
const stableBarrel = await readFile(new URL('infra/framework/src/index.js', root), 'utf8');
const optionalBarrel = await readFile(new URL('infra/framework/src/experimental.js', root), 'utf8');
...
assert.deepEqual(manifest.optionalFrameworkSurface.referenceConsumers, ['infra/framework/demo']);
