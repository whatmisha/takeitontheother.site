...
const [contractCss, controller, plan, ...entrypoints] = await Promise.all([
    read('infra/framework/css/ui-contract.css'),
    read('infra/framework/src/ui/UnifiedUiController.js'),
    read('infra/docs/G8_UI_CONTRACT_PLAN.md'),
...
assert.match(pizzaNavigation, /←\s+YF Tools/u, 'grid_generator: back link needs a readable arrow gap');
