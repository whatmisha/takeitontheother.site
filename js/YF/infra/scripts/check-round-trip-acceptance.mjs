...
const roundTrip = JSON.parse(await readFile(new URL('infra/ROUND_TRIP_ACCEPTANCE.json', root), 'utf8'));
const exportsManifest = JSON.parse(await readFile(new URL('infra/EXPORT_ACCEPTANCE.json', root), 'utf8'));
const capabilities = JSON.parse(await readFile(new URL('infra/APPLICATION_CAPABILITIES.json', root), 'utf8'));
