...
test('Sparky consumes the shared framework without surrendering private mobile and export behavior', async () => {
    const [toolSource, htmlSource, stylesSource, frameworkStylesSource] = await Promise.all([
        ...
        readFile(new URL('../../infra/framework/css/othersite-styles.css', import.meta.url), 'utf8')
    ]);
    ...
    const frameworkCss = htmlSource.indexOf('../infra/framework/css/othersite-styles.css');
    ...
    assert.match(
        htmlSource,
        /<a href="\.\.\/" class="top-link" aria-label="Back to YF Tools">← YF Tools<\/a>/u,
    ...
    assert.match(htmlSource, /framework\/css\/ui-contract\.css\?v=uiq-5/u, 'Sparky must load the shared system-font UI contract');
