...
export class PresetStore {
     ...
     * @param {string} options.storageKey — unique, versioned tool namespace
     * @param {number} [options.fetchTimeoutMs=5000] — seed request timeout
     */
    constructor({ storageKey, fetchTimeoutMs = 5000 } = {}) {
        const normalizedStorageKey = String(storageKey || '').trim();
        if (!normalizedStorageKey) {
            throw new TypeError('PresetStore requires a unique versioned storageKey');
        }
        this.storageKey = normalizedStorageKey;
        this.seedMarkerKey = `${normalizedStorageKey}__seeded`;
