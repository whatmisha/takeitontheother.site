...
const baseline = JSON.parse(await readFile(new URL('qa/migrations/SOURCE_BASELINE.json', root), 'utf8'));
const catalog = JSON.parse(await readFile(new URL('TOOL_CATALOG.json', root), 'utf8'));
const contracts = JSON.parse(await readFile(new URL('qa/migrations/CONTRACTS.json', root), 'utf8'));
const manifest = { schemaVersion: 1, scope: 'isolated-copy-not-acceptance', note: 'Original engines preserved except explicit storage namespaces. Only HTML/CSS dependency, UI font and preview-navigation edits. Full behavior/UI acceptance is pending.', namespaces, tools: [] };
const pending = [];
...
for (const id of ids) {
    ...
    try { await lstat(new URL(`tools/${id}/`, root)); throw Error(`${id}: target exists; refusing overwrite`); }
    ...
    for (const file of source.files) {
        ...
        if (file.path === 'index.html') {
            for (const [remote, local] of Object.entries(vendors)) {
                ...
                if (original.includes(remote)) await lstat(new URL(local, new URL(`tools/${id}/index.html`, root)));
            }
        ...
        const target = `tools/${id}/${file.path}`;
...
if (!process.argv.includes('--apply')) {
    ...
    for (const [file, data] of [['TOOL_CATALOG.json', catalog], ['qa/migrations/CONTRACTS.json', contracts], ['qa/migrations/BATCH7_COPY_MANIFEST.json', manifest]]) {
