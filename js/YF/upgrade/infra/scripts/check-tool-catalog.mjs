...
const legacy = JSON.parse(await readUpgrade('infra/APPLICATION_CAPABILITIES.json'));
const contracts = JSON.parse(await readUpgrade('infra/qa/migrations/CONTRACTS.json'));
const sources = JSON.parse(await readUpgrade('infra/qa/migrations/SOURCE_BASELINE.json'));
...
const diskDirectories = new Set((await readdir(upgradeRoot, { withFileTypes: true })).filter(item => item.isDirectory() && item.name !== 'infra').map(item => item.name));
...
for (const tool of catalog.tools) {
    ...
    assert.equal(tool.capabilityContract, `infra/qa/migrations/CONTRACTS.json#${tool.id}`);
