...
const manifest = JSON.parse(await read('infra/RUNTIME_RESILIENCE.json'));
...
const shell = await read('infra/framework/src/core/ApplicationShell.js');
...
const observed = await read('infra/framework/src/ui/ObservedControllerLifecycle.js');
...
const intake = await read('infra/framework/src/ui/FileIntakeController.js');
