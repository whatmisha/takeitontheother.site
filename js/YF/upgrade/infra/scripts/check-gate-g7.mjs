...
const [plan, resilience, live, runtimeDoc, liveDoc] = await Promise.all([
    read('infra/docs/G7_RELEASE_HARDENING_PLAN.md'),
    read('infra/RUNTIME_RESILIENCE.json'),
    read('infra/FINAL_LIVE_ACCEPTANCE.json'),
    read('infra/docs/RUNTIME_RESILIENCE_ACCEPTANCE.md'),
    read('infra/docs/G7_LIVE_ACCEPTANCE.md')
]);
