...
test('PresetStore writes only its isolated namespace and seed marker', () => {
    ...
    try {
        const store = new PresetStore({ storageKey: 'upgrade:test:presets:v1' });
        assert.deepEqual(store.create('One', { value: 1 }), { ok: true });
        store.markSeeded();
        assert.equal(store.load('One').value, 1);
        assert.equal(storage.getItem('upgrade:test:presets:v1__seeded'), '1');
...
test('PresetStore aborts stalled seed requests at its configured timeout', async () => {
    ...
    try {
        const store = new PresetStore({ storageKey: 'upgrade:timeout-test:v1', fetchTimeoutMs: 5 });
...
test('PresetSession keeps per-preset history and shared data ephemeral', () => {
    ...
    try {
        let state = { value: 1 };
        const store = new PresetStore({ storageKey: 'upgrade:session-test:presets:v1' });
