...
Автономная экспериментальная среда: восемь принятых инструментов и восемь новых migration preview.

**Текущая структура:** инструменты — `upgrade/<tool>/`, общая инфраструктура — `upgrade/infra/`. Команды npm выполняются из корня `upgrade`. [Карта папок и проверок](ROOT_LAYOUT.md), [план интеграции новых инструментов](REMAINING_YF_TOOLS_INTEGRATION_PLAN.md). Исторические отчёты ниже могут содержать прежние пути.
