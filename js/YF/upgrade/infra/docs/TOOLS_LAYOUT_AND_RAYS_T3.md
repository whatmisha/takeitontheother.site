...
> Исторический отчёт. Структура заменена решением [ROOT_LAYOUT.md](ROOT_LAYOUT.md): приложения теперь в `upgrade/<tool>/`, инфраструктура в `upgrade/infra/`. Результаты этапа Rays сохранены.
