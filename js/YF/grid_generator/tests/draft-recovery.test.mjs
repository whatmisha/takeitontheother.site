...
test('legacy draft import is insert-only and preserves dates and a newer saved draft', async () => {
    const store = new DraftStore({ indexedDBRef: new FakeIndexedDB() });
    const previous = { id: 'current', version: 1, savedAt: '2026-09-01T00:00:00Z', snapshot: { settings: {}, document: {} } };
    assert.deepEqual(await store.importIfEmpty(previous), previous);
    assert.deepEqual(await store.load(), previous);
    await store.save({ ...previous, presetName: 'Current' });
    const current = await store.load();
    assert.deepEqual(await store.importIfEmpty(previous), current);
