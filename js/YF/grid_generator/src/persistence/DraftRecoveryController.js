import { DraftStore } from './DraftStore.js?layout=root-infra-1';
import { importPreviousDraft } from './LegacyDraftMigration.js';
...
export class DraftRecoveryController {
    ...
    async checkForRecovery() {
        if (this.disposed) return false;
        const draft = await importPreviousDraft(this.store, await this.store.load());
